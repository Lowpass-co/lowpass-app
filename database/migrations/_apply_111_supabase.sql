-- Apply 111 — Rider Architecture Mirror (§RA1)
-- Hand-paste version for the Supabase SQL editor (-- comments only).
-- Idempotent: safe to re-run. (Don't paste this AND run the migration
-- runner with a tracking row — let one path own _lp_migrations.)
...
-- 3. Platform section template seeds (Contacts first; guarded) ----
DO $seed$
BEGIN
