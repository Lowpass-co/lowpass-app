...
**Second amendment (2026-07-04, Checkpoint A finding):** request-DTO field names at the API boundary (`body.show_rate` etc. — clients send these JSON keys) are ALSO allowed, narrowly: the route maps them to the camelCase `writeRates`/lib input in one block at the top of the handler; nothing downstream of that mapping names them. Wire-contract keys are not DB-column knowledge; renaming the wire contract is out of scope (revisit, if ever, during the design pass UI conversions).
