/* ============================================
   LOWPASS — POST /api/gear/export/carnet/pdf  (S1 D-1)

   Streams the ATA carnet GENERAL LIST — the schedule of goods a carnet
   APPLICATION requires. NOT a carnet: those are issued by a chamber of
   commerce, and the document itself says so on its face.
...
import { buildCarnetExport } from '@/lib/export/build';
...
export async function POST(request: Request): Promise<NextResponse | Response> {
  ...
  try {
    ...
    const config = normalizeConfig('carnet', body.config);
    ...
    return exportPdfResponse('carnet', async () => {
      const { html, footerNote, filename, footer, runningHeader } = await buildCarnetExport(
        supabase,
    ...
    return exportErrorResponse('carnet', err);
  }
}
