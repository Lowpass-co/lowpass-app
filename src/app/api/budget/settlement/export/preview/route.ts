/* ============================================
   LOWPASS — POST /api/budget/settlement/export/preview  (M1-B · SET-05)

   Returns { html } for the live settlement-Walk preview — byte-identical to the PDF
   body (same buildSettlementExport). Body: { routing_id, config? }.
   ============================================ */
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const { html } = await buildSettlementExport(
      supabase,
    ...
    return exportErrorResponse('settlement-preview', err);
  }
}
