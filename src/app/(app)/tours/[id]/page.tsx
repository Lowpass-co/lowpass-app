/* ============================================
   LOWPASS — Tour Hub (Phase C nav redesign)

   The working hub for a single tour. Layout:
     ...
     Setup strip     Routing / Channel list / Personnel / Rooming /
                     Riders linked
     Primary CTAs    Advance card | Budget card    (2px orange border)
     Timeline        TourOverviewClient (UX16) — secondary, wrapped in
                     bg-secondary card so it reads as the day-to-day
                     plan rather than the headline of the page.
     Secondary       Personnel · Routing · Channel list · Rooming
                     small cards.

   Data flows through src/server/tours/getTourHubData.ts. The
   TimelineDashboard data (shows/hotels/flights) is fetched inline
   to keep TourOverviewClient's prop shape unchanged.
   ============================================ */
...
/** Compact currency formatter — £42K, $1.2M, etc. */
...
/** Budget bar colour: green under 80%, amber 80–100%, red over 100%. */
...
export default async function TourHubPage({
  ...
  // TourOverviewClient. Same shape as the previous /tours/[id]
  // implementation — kept inline so this page owns its render.
  const [routingRes, hotelRes, flightRes, personnelCountRes, budgetSumRes] =
    await Promise.all([
      supabase
        .from('routing')
      ...
      supabase
        ...
        .eq('tour_id', tourId),
      // Reuse hub's proposed total for the timeline header — Tour
      // Overview reads it as a single number.
      Promise.resolve({ value: hub.budget.proposed }),
    ]);
    ...
    <div className="mx-auto w-full max-w-7xl space-y-6">
      ...
      >
        ...
        <TourPrimaryCTACard
          ...
          // out of the card; the percent label still reads honestly.
      ...
      <section className="space-y-2">
        ...
        >
          <TourOverviewClient
            ...
            totalBudget={budgetSumRes.value || null}
            shows={shows}
            hotels={hotels}
            flights={flights}
          />
