...
/* ============================================
   LOWPASS — Tour Breadcrumb
   Built on the budget-redesign fix-up branch (X3) — PR #3 originally
   specified this component but never shipped it.

       [← Artist › Tour › Page]                  [Back to tour]

   Sticky top:0 inside the page's <main> scroll container. Each
   segment is clickable except the current page. Print stylesheet
   hides it (chrome, not content).
   ============================================ */
...
function derivePageLabel(pathname: string, tourId: string): string | null {
  ...
  // Hub itself doesn't render the sticky breadcrumb — the hub has its
  // own ← Artist link in the page body (see TourHubClient).
