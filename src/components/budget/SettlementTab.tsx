          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-lp-border bg-lp-bg-tertiary">
                <th className="text-left p-3 font-medium text-lp-text">Date</th>
                <th className="text-left p-3 font-medium text-lp-text">Venue</th>
                <th className="text-left p-3 font-medium text-lp-text">City</th>
                <th className="text-left p-3 font-medium text-lp-text">Status</th>
                <th className="text-right p-3 font-medium text-lp-text">Day-of Net</th>
                <th className="text-right p-3 font-medium text-lp-text">Reconciled Net</th>
          ...
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl border border-lp-border bg-lp-surface p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-6">
              <div>
                ...
                <select className="w-full rounded border border-lp-border bg-lp-bg px-3 py-2 text-lp-text" value={form.status ?? 'pending'} onChange={(e) => setForm((f) => ({ ...f!, status: e.target.value }))}>
              ...
              <div><label className="block text-sm font-medium text-lp-text mb-1">Signed by</label><input type="text" className="w-full rounded border border-lp-border bg-lp-bg px-3 py-2 text-lp-text" value={form.day_of_signed_by ?? ''} onChange={(e) => setForm((f) => ({ ...f!, day_of_signed_by: e.target.value || null }))} placeholder="TM initials or name" /></div>
              <div><label className="block text-sm font-medium text-lp-text mb-1">Day-of Notes</label><textarea className="w-full rounded border border-lp-border bg-lp-bg px-3 py-2 text-lp-text min-h-[60px]" value={form.day_of_notes ?? ''} onChange={(e) => setForm((f) => ({ ...f!, day_of_notes: e.target.value || null }))} /></div>
              ...
              <div><label className="block text-sm font-medium text-lp-text mb-1">Reconciled Notes</label><textarea className="w-full rounded border border-lp-border bg-lp-bg px-3 py-2 text-lp-text min-h-[60px]" value={form.reconciled_notes ?? ''} onChange={(e) => setForm((f) => ({ ...f!, reconciled_notes: e.target.value || null }))} /></div>

              <hr className="border-lp-border" />

              <div><label className="block text-sm font-medium text-lp-text mb-1">Deal Memo (text)</label><textarea className="w-full rounded border border-lp-border bg-lp-bg px-3 py-2 text-lp-text min-h-[80px]" value={form.deal_memo_text ?? ''} onChange={(e) => setForm((f) => ({ ...f!, deal_memo_text: e.target.value || null }))} placeholder="Deal memo summary or paste" /></div>
