...
export function DayViewTab({ tourId }: { tourId: string }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [routingRes, incomeRes, liRes, tourRes] = await Promise.all([
        ...
        // dates as Salaries/Commissions and the Routing editor.
  ...
  const incomeLines = useMemo(() => {
    ...
    const actualGuarantee = n(incomeRow.actual_guarantee) + n(incomeRow.actual_overage);
    ...
    if (withholdingDeduction > 0) {
      lines.push({ label: 'Withholding tax', proposed: -withholdingDeduction, actual: 0 });
    }
    return lines;
  }, [incomeRow]);
