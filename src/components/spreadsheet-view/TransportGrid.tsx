...
const TRANSPORT_CATEGORIES: { key: string; label: string }[] = [
  { key: 'transport_bus', label: 'BUS + TRUCK' },
  { key: 'transport_taxis', label: 'TAXIS' },
  { key: 'transport_misc', label: 'MISC TRANSPORT' },
  { key: 'transport_fuel', label: 'FUEL' },
  { key: 'transport_parking', label: 'PARKING' },
  { key: 'transport_agent', label: 'TRAVEL AGENT' },
];

export function TransportGrid({ tourId, currency }: { tourId: string; currency: string }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setLineItems(all.filter((i: BudgetLineItem) => i.category.startsWith('transport_')));
    } catch (e) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {TRANSPORT_CATEGORIES.map(({ key: cat, label }) => {
