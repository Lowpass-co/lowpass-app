...
function renderStatement(p: PayrollPerson, ccy: string): string {
  ...
  const breakdown = [
    ...
    Math.abs(advance) >= 1 ? ['Advance', '', advance] : null,
  ...
  return `
    ...
    <div class="lp-native" style="margin:-2px 0 6px;">${[esc(p.role ?? 'Crew'), esc(ccy)].join('  ·  ')}</div>
    <table class="lp-tbl">
...
function renderStatements(data: PayrollExportData): string {
  if (data.persons.length === 0) return '';
  return data.persons.map((p) => renderStatement(p, data.currency)).join('');
}

/** P1+P2 template builder. `config.sections` (order + visibility) drives WHICH
 *  blocks render. DEFAULT (run-sheet then statements) is the standard payroll PDF. */
export function buildPayrollBodyHtml(data: PayrollExportData, config: TemplateConfig): string {
  const sections: Record<string, () => string> = {
    ...
    statements: () => renderStatements(data),
  };
